...
import {
  SafeAreaView, View, Text, TouchableOpacity, FlatList, Alert, Platform,
  RefreshControl, TextInput, NativeScrollEvent, NativeSyntheticEvent
} from 'react-native';
...
type ItemDoc = {
  name?: string; lastCount?: number; lastCountAt?: any;
...
function DepartmentVarianceScreen() {
  ...
  const load = useCallback(async () => {
    ...
    for (const a of areasSnap.docs) {
      ...
      itemsSnap.forEach(i => {
        ...
        if (exp == null) expectedSum = null;
        else if (expectedSum != null) { expectedSum += exp; haveExpected = true; }
  ...
  const viewRows = useMemo(() => {
    ...
    if (sortByMagnitude)
      r.sort((a, b) => Math.abs((b.variance ?? 0)) - Math.abs((a.variance ?? 0)));
    else
      r.sort((a, b) => a.areaName.localeCompare(b.areaName));
    return r;
  ...
  const copyCsv = async (mode: 'current' | 'changes') => {
    try {
      ...
      const dataset = mode === 'changes'
        ? viewRows.filter(r => (r.variance ?? 0) !== 0)
        : viewRows;
  ...
  const exportCsv = async (mode: 'current' | 'changes') => {
    try {
      ...
      if (Sharing?.isAvailableAsync && await Sharing.isAvailableAsync()) {
        await Sharing.shareAsync(path, { mimeType: 'text/plain', dialogTitle: fname });
      } else {
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: '#FFFFFF' }}>
      <View style={{ padding: 16 }}>
        ...
        <Text style={{ color: '#6B7280', marginBottom: 10 }}>
          Sums Expected vs Counted for each area in this department. Variance = Counted − Expected.
        </Text>
        ...
        <View style={{ gap: 8, marginBottom: 8 }}>
          ...
          <View style={{ flexDirection: 'row', gap: 8, flexWrap: 'wrap' }}>
            <TouchableOpacity
              ...
              style={{ paddingVertical: 6, paddingHorizontal: 12, borderRadius: 16, borderWidth: 1, borderColor: onlyVariance ? '#1D4ED8' : '#E5E7EB', backgroundColor: onlyVariance ? '#DBEAFE' : 'white' }}>
              <Text style={{ fontWeight: '800', color: onlyVariance ? '#1D4ED8' : '#374151' }}>
                {onlyVariance ? '✓ Non-zero variance' : 'Non-zero variance'}
              </Text>
            </TouchableOpacity>

            <TouchableOpacity
              onPress={() => setSortByMagnitude(v => !v)}
              style={{ paddingVertical: 6, paddingHorizontal: 12, borderRadius: 16, borderWidth: 1, borderColor: sortByMagnitude ? '#059669' : '#E5E7EB', backgroundColor: sortByMagnitude ? '#D1FAE5' : 'white' }}>
              <Text style={{ fontWeight: '800', color: sortByMagnitude ? '#065F46' : '#374151' }}>
                {sortByMagnitude ? '✓ Sort by largest variance' : 'Sort A–Z'}
              </Text>
            </TouchableOpacity>

            {/* Delimiter toggle */}
            <TouchableOpacity
              onPress={() => setDelimiter('comma')}
              style={{ paddingVertical: 6, paddingHorizontal: 12, borderRadius: 16, borderWidth: 1, borderColor: delimiter === 'comma' ? '#1E40AF' : '#E5E7EB', backgroundColor: delimiter === 'comma' ? '#DBEAFE' : 'white' }}>
              <Text style={{ fontWeight: '800', color: delimiter === 'comma' ? '#1E40AF' : '#374151' }}>
                {delimiter === 'comma' ? '✓ CSV (,)' : 'CSV (,)'}
              </Text>
            </TouchableOpacity>
            <TouchableOpacity
              onPress={() => setDelimiter('tab')}
              style={{ paddingVertical: 6, paddingHorizontal: 12, borderRadius: 16, borderWidth: 1, borderColor: delimiter === 'tab' ? '#1E40AF' : '#E5E7EB', backgroundColor: delimiter === 'tab' ? '#DBEAFE' : 'white' }}>
              <Text style={{ fontWeight: '800', color: delimiter === 'tab' ? '#1E40AF' : '#374151' }}>
                {delimiter === 'tab' ? '✓ TSV (tab)' : 'TSV (tab)'}
              </Text>
            </TouchableOpacity>
            ...
            {anyFilter ? (
              <TouchableOpacity
                onPress={() => { setSearch(''); setOnlyVariance(false); setSortByMagnitude(true); }}
                style={{ paddingVertical: 6, paddingHorizontal: 12, borderRadius: 16, borderWidth: 1, borderColor: '#E5E7EB', backgroundColor: '#F3F4F6' }}>
      ...
      {showTop ? (
        <TouchableOpacity
          ...
          style={{
            ...
            bottom: Platform.select({ ios: 72, android: 64 }),
            backgroundColor: '#111827', paddingVertical: 10, paddingHorizontal: 14, borderRadius: 20
          }}>
