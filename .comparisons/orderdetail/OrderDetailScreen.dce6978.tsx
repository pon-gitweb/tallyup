...
export default function OrderDetailScreen() {
  ...
  // Common persist step after parse (CSV/PDF)
  ...
  // ---------- Unconditional effect; guarded body ----------
  useEffect(() => {
    if (!csvReview || autoConfirmedRef.current) return;
    const tier = tierForConfidence(csvReview.confidence);
    if (tier !== 'high') return;
    ...
    (async () => {
      try {
        ...
        autoConfirmedRef.current = false; // allow retry if it fails
        Alert.alert('Auto-receive failed', String((e as any)?.message || e));
      }
    })();
          ...
          renderItem={({ item }) => (
            <View style={S.line}>
              ...
              <Text style={{ color: '#6B7280' }}>
                Qty: {item.qty ?? 0} • Unit: ${Number(item.unitCost || 0).toFixed(2)}
              </Text>
            </View>
          )}
        />
