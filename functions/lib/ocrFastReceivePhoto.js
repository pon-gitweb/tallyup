...
function extractInvoiceNumber(text) {
    ...
    if (fallback?.[0])
        return fallback[0].toUpperCase().slice(0, 64);
    return null;
}
...
function extractDeliveryDate(text) {
    ...
    for (const raw of lines) {
        ...
        const isInteresting = lower.includes("delivery") ||
            lower.includes("date") ||
...
function guessSupplierName(text) {
    ...
    for (const raw of lines.slice(0, 12)) {
        ...
        if (lower.includes("statement"))
            continue;
        if (lower.includes("po #") || lower.includes("po #:"))
            continue;
        if (lower.includes("item qty") || lower.includes("qty unit"))
            continue;
        if (lower.includes("subtotal") || lower.includes("gst"))
            continue;
        if (lower.includes("total (incl"))
            continue;
        if (lower.includes("page"))
            continue; // avoid "1 page"
        // Must contain letters, not just numbers
        if (!/[A-Za-z]/.test(line))
            continue;
        // Avoid lines that are probably addresses/phones only
        if (/\b\d{3,}\b/.test(line) && !/[A-Za-z]{3,}/.test(line))
            continue;
    ...
    const preferred = candidates.find((c) => /(foods|foodservice|distributors|limited|ltd|wholesale|suppl|nz)\b/i.test(c));
    if (preferred)
        return preferred;
...
function extractLines(text) {
    ...
    const start = tableStart >= 0 ? tableStart : qtyHeaderIndex >= 0 ? qtyHeaderIndex : 0;
...
function makeLinesFirestoreSafe(lines) {
    return (lines || []).map((l) => {
        const safe = {
