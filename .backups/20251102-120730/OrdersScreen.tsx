  ...
  const renderItem=useCallback(({item}:{item:OrderRow})=>{
    ...
    return(
      <View style={S.row}>
        ...
        {/* Receive button only for submitted orders */}
        {isSubmitted ? (
          <TouchableOpacity
            style={S.smallBtn}
            onPress={()=>startReceive(item)}
            activeOpacity={0.9}
          >
        ...
        <TouchableOpacity activeOpacity={1} style={{flex:1,justifyContent:'flex-end',backgroundColor:'rgba(0,0,0,0.3)'}} onPress={()=>setReceiveFor(null)}>
          <View style={{backgroundColor:'#fff',padding:16,borderTopLeftRadius:16,borderTopRightRadius:16}}>
            ...
            <TouchableOpacity style={{paddingVertical:12}} onPress={()=>{
              ...
              setReceiveFor(null);
              // PHASE-1: PDF only
              if(id) nav.navigate('OrderDetail',{orderId:id, receiveNow:true, receiveMode:'upload_pdf'});
            }}>
              <Text style={{fontSize:16,fontWeight:'700'}}>Upload invoice (PDF)</Text>
            </TouchableOpacity>
