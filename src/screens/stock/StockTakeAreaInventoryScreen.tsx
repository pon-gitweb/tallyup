...
import {
  ...
  TextInput, FlatList, Modal, Pressable, ActivityIndicator, ScrollView, Switch, Platform
} from 'react-native';
...
export default function StockTakeAreaInventoryScreen() {
  ...
  const [liveItems, setLiveItems] = useState<ItemRow[]>([]);
  ...
  // ——— CREATE ———
  async function addItemQuickNameOnly(name: string) {
    if (!venueId) return;
    const n = name.trim();
    if (!n) throw new Error('Missing item name');
    const colRef = collection(db, 'venues', venueId, 'departments', departmentId, 'areas', areaId, 'items');
    const now = serverTimestamp();
    const body: any = { name: n, createdAt: now, updatedAt: now };
  ...
  // Footer: Quick Add (name-only), expected toggle, and "Add with full details"
  function ListFooter() {
    ...
    return (
      <View style={styles.footerWrap}>
        <Text style={styles.footerTitle}>Quick add (name only)</Text>
        <View style={styles.fastAddRow}>
          <TextInput value={name} onChangeText={setName} placeholder="Item name" style={[styles.input, { flex: 1 }]} />
    ...
    <View style={styles.wrap}>
      ...
      {isLoading ? (
        ...
        <FlatList
          data={items}
          keyExtractor={(i) => i.id}
          contentContainerStyle={{ paddingBottom: 140 }}  // space for sticky submit
          ...
          ListEmptyComponent={
            <View style={{ padding: 12 }}>
              <Text>No items yet. Add your first product below.</Text>
              <ListFooter />
            </View>
          }
        />
      ...
      <View style={styles.bottomBar} pointerEvents="box-none">
        <TouchableOpacity style={styles.bottomSubmitBtn} onPress={onSubmit}>
          <Text style={styles.bottomSubmitText}>Submit Area</Text>
        </TouchableOpacity>
      </View>
...
const styles = StyleSheet.create({
  ...
  modalInput: { borderWidth: 1, borderColor: '#D0D3D7', borderRadius: 10, paddingHorizontal: 12, paddingVertical: 10, marginTop: 8, backgroundColor: 'white' },

  // footer (Quick Add / toggle / link)
  footerWrap: { backgroundColor: '#eef6ff', padding: 12, borderRadius: 12, marginTop: 10 },
  footerTitle: { fontWeight: '800', marginBottom: 8, color: '#0A84FF' },
  ...
  bottomSubmitBtn: { backgroundColor: '#0A84FF', paddingVertical: 14, borderRadius: 12, alignItems: 'center', justifyContent: 'center' },
  bottomSubmitText: { color: 'white', fontWeight: '800', fontSize: 16 },
});
