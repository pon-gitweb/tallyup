...
function TermsScreen() {
  ...
  const Section = ({ title, children }: any) => (
    <View style={{ marginBottom: 16 }}>
      <Text style={{ fontWeight: '900', color: themeColours.text, fontSize: 15, marginBottom: 6 }}>{title}</Text>
      <Text style={{ color: themeColours.textSecondary, fontSize: 13, lineHeight: 20 }}>{children}</Text>
    </View>
  );

  return (
    <ScrollView style={{ flex: 1, backgroundColor: themeColours.background }} contentContainerStyle={{ padding: 16, gap: 4 }}>

      <View style={{ backgroundColor: themeColours.primary, borderRadius: 16, padding: 20, marginBottom: 16 }}>
