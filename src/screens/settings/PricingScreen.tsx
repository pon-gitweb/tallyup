...
function PricingScreen() {
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: C.background }} contentContainerStyle={{ padding: 16, gap: 16 }}>
      ...
      {/* Header */}
      <View style={{ backgroundColor: C.primary, borderRadius: 16, padding: 20, alignItems: 'center', gap: 6 }}>
      ...
      {PLANS.map((plan, i) => (
        <View key={i} style={{
          ...
          borderWidth: plan.badge ? 2 : 1, borderColor: plan.badge ? plan.colour : C.border,
        }}>
          {plan.badge && (
          ...
          <View style={{ padding: 16 }}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
              <Text style={{ fontSize: 20, fontWeight: '900', color: C.text }}>{plan.name}</Text>
              <View style={{ alignItems: 'flex-end' }}>
                <Text style={{ fontSize: 28, fontWeight: '900', color: plan.colour }}>{plan.price}</Text>
                {plan.period ? <Text style={{ color: C.textSecondary, fontSize: 12 }}>{plan.period}</Text> : null}
              </View>
            </View>
      ...
      {/* Contact */}
      <View style={{ backgroundColor: C.surface, borderRadius: 14, padding: 14, borderWidth: 1, borderColor: C.border, alignItems: 'center', gap: 8 }}>
        <Text style={{ fontWeight: '800', color: C.text }}>Questions about pricing?</Text>
