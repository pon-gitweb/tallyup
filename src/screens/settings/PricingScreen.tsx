...
function PricingScreen() {
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: themeColours.background }} contentContainerStyle={{ padding: 16, gap: 16 }}>
      ...
      {/* Header */}
      <View style={{ backgroundColor: themeColours.primary, borderRadius: 16, padding: 20, alignItems: 'center', gap: 6 }}>
      ...
      {PLANS.map((plan, i) => (
        <View key={i} style={{
          backgroundColor: themeColours.surface, borderRadius: 16, overflow: 'hidden',
          borderWidth: plan.badge ? 2 : 1, borderColor: plan.badge ? plan.colour : themeColours.border,
        }}>
          {plan.badge && (
          ...
          <View style={{ padding: 16 }}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
              <Text style={{ fontSize: 20, fontWeight: '900', color: themeColours.text }}>{plan.name}</Text>
              <View style={{ alignItems: 'flex-end' }}>
                <Text style={{ fontSize: 28, fontWeight: '900', color: plan.colour }}>{plan.price}</Text>
                {plan.period ? <Text style={{ color: themeColours.textSecondary, fontSize: 12 }}>{plan.period}</Text> : null}
              </View>
            </View>
            {plan.features.map((f, j) => (
              <View key={j} style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                ...
                <Text style={{ color: themeColours.text, fontSize: 14 }}>{f}</Text>
              </View>
            ))}
          </View>
        </View>
      ...
      {/* Contact */}
      <View style={{ backgroundColor: themeColours.surface, borderRadius: 14, padding: 14, borderWidth: 1, borderColor: themeColours.border, alignItems: 'center', gap: 8 }}>
        <Text style={{ fontWeight: '800', color: themeColours.text }}>Questions about pricing?</Text>
