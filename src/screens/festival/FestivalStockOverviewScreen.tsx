  ...
  useEffect(() => {
    ...
    const unsubBars = onSnapshot(
      ...
      async barSnap => {
        ...
        const promises = barSnap.docs.map(async barDoc => {
          const barData = barDoc.data() as any;
            ...
            onSnapshot(
              ...
              stockSnap => {
                const items: StockItem[] = stockSnap.docs.map(d => ({
                  productId: d.id,
                  productName: (d.data() as any).productName || d.id,
                  currentStock: (d.data() as any).currentStock ?? 0,
                  stockCategory: (d.data() as any).stockCategory,
                }));
                const existing = results.find(r => r.locationId === barDoc.id);
                if (existing) {
                  existing.items = items;
                } else {
                  results.push({ locationId: barDoc.id, locationName: barData.name || barDoc.id, items });
                }
                barsData = results;
    ...
    const unsubSrc = onSnapshot(
      ...
      async srcSnap => {
        const results: LocationStock[] = [];
        const promises = srcSnap.docs.map(async srcDoc => {
          const srcData2 = srcDoc.data() as any;
          return new Promise<void>(resolve => {
            onSnapshot(
              collection(db, 'venues', venueId, 'sourceLocations', srcDoc.id, 'stock'),
              stockSnap => {
                const items: StockItem[] = stockSnap.docs.map(d => ({
                  productId: d.id,
                  productName: (d.data() as any).productName || d.id,
                ...
                const existing = results.find(r => r.locationId === srcDoc.id);
                if (existing) {
                  existing.items = items;
                } else {
                  results.push({ locationId: srcDoc.id, locationName: srcData2.name || srcDoc.id, items });
                }
                srcData = results;
