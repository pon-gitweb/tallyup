...
export default function SuggestedOrderScreen(){
  ...
  const doRefreshRaw=useCallback(async()=>{ if(!venueId){ setRows([]); setSnapshot(null); return; } await loadExistingSuggestionKeys();
    ...
    const graduated=await graduateUnassignedUsingProducts(venueId, norm.buckets, norm.unassigned); await computeRowsFromSnapshot(graduated);
  ...
  const onToggleMode = useCallback(async(nextMode:'math'|'ai')=>{
    ...
    try{
      if(nextMode==='math'){
        ...
        const graduated=await graduateUnassignedUsingProducts(venueId, res.buckets, res.unassigned);
  ...
  const renderRow=useCallback(({item:row}:{item:BucketRow})=>(
    <TouchableOpacity style={[S.row,row.supplierId==='unassigned'?S.rowUnassigned:null]} onPress={()=> row.supplierId==='unassigned'? setUnassignedOpen(true) : openSupplierPreview(row.supplierId,row.supplierName)}>
      <View style={{flex:1}}>
  ...
  // NEW: Segmented control + meter, replaces the old "Use AI" button
  ...
  const listHeader=useMemo(()=>(
    <View style={S.header}>
      <View style={{flex:1}}><Text style={S.title}>Suggested Orders</Text><Text style={S.rowSub}>Math first. Use AI when you want.</Text></View>
      <IdentityBadge/>
    </View>
  ),[]);
