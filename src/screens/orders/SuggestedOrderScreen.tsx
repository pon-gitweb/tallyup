...
export default function SuggestedOrderScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      // minimal example: group by supplier from suggestedOrders collection
      const ref = collection(db, 'venues', venueId, 'suggestedOrders');
      const snap = await getDocs(ref);
      setRows(snap.docs.map(d => ({ id: d.id, ...(d.data() as any) })));
    } catch (e:any) {
      if (__DEV__) console.log('[SuggestedOrders] load error', e?.message);
      setRows([]);
    }
  }, [db, venueId]);
  ...
  const createAll = () => {
    ...
    // Wire your existing create-drafts service here if available.
  };
  ...
  return (
    <View style={S.wrap}>
      ...
      <FlatList
        ...
        ListEmptyComponent={<View style={{ padding: 16 }}><Text style={{ color: '#6B7280' }}>No suggestions yet.</Text></View>}
      />
    </View>
  );
}
