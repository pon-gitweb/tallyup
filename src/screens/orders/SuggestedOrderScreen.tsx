...
// Math suggester (fallback)
import { buildSuggestedOrdersInMemory } from '../../services/orders/suggest';

// AI suggester + entitlement (optional)
import { buildSuggestedOrdersAI } from '../../services/orders/suggestAI';
import { USE_AI_SUGGESTER } from '../../config/ai';
...
// Assign supplier on PRODUCT (Option B — do not modify closed area items)
...
export default function SuggestedOrderScreen(){
  ...
  // Accept both shapes from suggester, then normalize & de-dupe
  ...
  // Option B graduation shim: re-check Products for unassigned lines and graduate those with supplier
  async function graduateUnassignedUsingProducts(
    ...
    // 2) Graduate lines that now have a product supplier
  ...
  const doRefresh = useCallback(async ()=>{
    ...
    setRefreshing(true);
    try{
      // Refresh existing keys first (so preview can block duplicates)
      await loadExistingSuggestionKeys();

      // Determine entitlement (safe default: not allowed)
      let entitlement = { allowed: false, allowedHistoryDays: 0 as number };
      try {
        entitlement = await checkEntitlement(venueId);
      } catch {
        entitlement = { allowed: false, allowedHistoryDays: 0 };
      }

      // 1) Try AI if globally enabled and entitlement allows; otherwise math suggester
      let compat:any = null;
      if (USE_AI_SUGGESTER && entitlement.allowed) {
        try {
          compat = await buildSuggestedOrdersAI(venueId, { roundToPack: true, defaultParIfMissing: 6, historyDays: entitlement.allowedHistoryDays });
        } catch {
          compat = await buildSuggestedOrdersInMemory(venueId, { roundToPack: true, defaultParIfMissing: 6 });
        }
      } else {
        compat = await buildSuggestedOrdersInMemory(venueId, { roundToPack: true, defaultParIfMissing: 6 });
      }
      ...
      // GRADUATE Unassigned lines using current Products (Option B)
      const graduated = await graduateUnassignedUsingProducts(venueId, buckets, unassigned);
      buckets = graduated.buckets;
      unassigned = graduated.unassigned;
  ...
  const createDraftForPreview = useCallback(async () => {
    ...
    try {
      ...
      // Confirmation only (no navigation)
      ...
      // Update local block list so button disables immediately
  ...
  const pickSupplier = useCallback(async (s:{id:string;name:string})=>{
    ...
    try{
      ...
      await doRefresh(); // counts + lines update after graduation
    }catch(e:any){
  ...
  const listHeader = useMemo(()=>(
    <View style={S.header}><Text style={S.title}>Suggested Orders</Text><IdentityBadge/></View>
  ),[]);
