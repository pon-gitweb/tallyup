...
export default function StocktakeCycleDetailScreen() {
  ...
  const filteredItems = useMemo(() => {
    ...
    return [...result].sort((a, b) => {
      ...
      if (sort === 'value') return (Math.abs((b.displayTotalVarianceDollars ?? b.totalVarianceDollars) ?? 0)) - (Math.abs((a.displayTotalVarianceDollars ?? a.totalVarianceDollars) ?? 0));
  ...
  async function exportPdf() {
    ...
    try {
      ...
      const hasInvoiceItems = (snapshot.summary?.itemsPricedByInvoice ?? 0) > 0;
      const rowsHtml = items.map(it => {
        const displayVar = it.displayTotalVarianceDollars ?? it.totalVarianceDollars;
        const isInvoice = it.costPriceTier === 'invoice_verified';
        return `
        <tr>
          <td>${it.name || ''}${isInvoice ? ' *' : ''}</td>
          <td>${it.areaName || ''}</td>
          ...
          <td style="text-align:right">${displayVar != null ? (displayVar < 0 ? '-' : '+') + '$' + Math.abs(displayVar).toFixed(0) : '–'}</td>
        </tr>`;
      }).join('');
...
.legend{margin-top:8px;font-size:10px;color:#6b7280;}
...
<div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:12px;margin-bottom:16px;">
  ...
  Products: ${snapshot.summary?.totalItemsCounted ?? items.length}${(snapshot.summary?.displayTotalStockValue ?? snapshot.summary?.totalStockValue) != null ? ' · Stock value: ' + fmtVal(snapshot.summary.displayTotalStockValue ?? snapshot.summary.totalStockValue) : ''}${(snapshot.summary?.displayTotalVarianceDollars ?? snapshot.summary?.totalVarianceDollars) != null ? ' · Variance: ' + fmtVal(snapshot.summary.displayTotalVarianceDollars ?? snapshot.summary.totalVarianceDollars) : ''}
</div>
<table>
...
${hasInvoiceItems ? '<div class="legend">* Price sourced from matched invoice (not stamped in product catalogue)</div>' : ''}
      ...
      <ScrollView contentContainerStyle={s.scroll}>
        ...
        <View style={s.card}>
          ...
          {[
            ...
            ['Stock value', (sum.displayTotalStockValue ?? sum.totalStockValue) != null ? fmtVal(sum.displayTotalStockValue ?? sum.totalStockValue) : 'No prices set'],
            ['Total variance', sum.totalVarianceQty != null ? `${sum.totalVarianceQty > 0 ? '+' : ''}${sum.totalVarianceQty} units` : '–'],
            ['Dollar variance', (sum.displayTotalVarianceDollars ?? sum.totalVarianceDollars) != null ? fmtVal(sum.displayTotalVarianceDollars ?? sum.totalVarianceDollars) : 'No prices set'],
            ...(sum.itemsPricedByInvoice > 0 ? [['Priced via invoice 📄', `${sum.itemsPricedByInvoice} item${sum.itemsPricedByInvoice !== 1 ? 's' : ''}`]] : []),
              ...
              <View style={[s.card, { flex: 1 }]}>
                ...
                {topLosses.map((it, i) => (
                  <View key={i} style={[s.varRow, i > 0 && s.varRowBorder]}>
                    ...
                    {(it.displayTotalVarianceDollars ?? it.totalVarianceDollars) != null && (
                      <Text style={s.varDollar}>–{fmtVal(Math.abs(it.displayTotalVarianceDollars ?? it.totalVarianceDollars))}{it.costPriceTier === 'invoice_verified' ? ' 📄' : ''}</Text>
                    )}
                  </View>
                ))}
              </View>
              ...
              <EditableItemRow key={it.productId || i} style={[s.itemRow, i > 0 && s.itemRowBorder]}>
                <View style={{ flex: 1 }}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                    <Text style={s.itemName}>{it.name}{it.costPriceTier === 'invoice_verified' ? ' 📄' : ''}</Text>
                ...
                <View style={{ alignItems: 'flex-end' }}>
                  ...
                  {(it.displayTotalVarianceDollars ?? it.totalVarianceDollars) != null && (it.displayTotalVarianceDollars ?? it.totalVarianceDollars) !== 0 && (
                    <Text style={[s.itemDollar, { color: varColor }]}>
                      {(it.displayTotalVarianceDollars ?? it.totalVarianceDollars) < 0 ? '-' : '+'}${Math.abs(it.displayTotalVarianceDollars ?? it.totalVarianceDollars).toFixed(0)}{it.costPriceTier === 'invoice_verified' ? ' 📄' : ''}
                    </Text>
                  )}
                </View>
              </EditableItemRow>
