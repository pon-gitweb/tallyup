...
import { getFirestore, collection, collectionGroup, getDocs, doc, getDoc, query, orderBy, limit, where } from 'firebase/firestore';
...
export default function ReconciliationCard({ venueId: propVenueId, onOpenOrder, maxSuppliers = 20, maxPerSupplier = 25 }: Props) {
  ...
  const load = useCallback(async ()=>{
    ...
    try {
      ...
      // Preferred path: collectionGroup on 'reconciliations' filtered by venueId if stored
      // Many of our saves include venueId for CG queries; if not present, we fallback to per-order scan.
      let usedGroup = false;
      try {
        const cg = query(
          collectionGroup(db, 'reconciliations'),
          where('venueId', '==', venueId),
          orderBy('createdAt', 'desc'),
          limit(maxSuppliers * maxPerSupplier) // crude cap; we reshape below
        );
        const cgSnap = await getDocs(cg);
        cgSnap.forEach(d=>{
          const v:any = d.data() || {};
          // parent path: venues/{venueId}/orders/{orderId}/reconciliations/{id}
          const path = d.ref.path || '';
          const m = path.match(/venues\/[^/]+\/orders\/([^/]+)\/reconciliations\/([^/]+)/);
          const orderId = (m && m[1]) || v.orderId || '';
          out.push({
            id: d.id,
            orderId,
            supplierName: v.supplierName ?? v.orderSupplierName ?? null,
            supplierId: v.supplierId ?? null,
            createdAt: v.createdAt || v.ts || null,
        ...
        if (__DEV__) console.log('[ReconciliationCard] collectionGroup not available or missing indexes, falling back', e?.message);
      }

      if (!usedGroup) {
        // Fallback: scan recent orders then grab last few reconciliations for each
        const ordersQ = query(
  ...
  return (
    <View style={{ backgroundColor:'#0B1220', borderRadius:12, overflow:'hidden' }}>
      ...
      <View style={{ padding:16, borderBottomColor:'#1F2A44', borderBottomWidth:1 }}>
        ...
        {/* Search */}
        <View style={{ marginTop:12, backgroundColor:'#0F172A', borderColor:'#1E293B', borderWidth:1, borderRadius:10, paddingHorizontal:12, paddingVertical:8 }}>
          <TextInput
            value={qText}
                ...
                return (
                  ...
                  >
                    <View style={{ flexDirection:'row', justifyContent:'space-between', alignItems:'center' }}>
                      ...
                      <View style={{ paddingHorizontal:8, paddingVertical:4, borderRadius:999, borderWidth:1, borderColor:st.color }}>
                        <Text style={{ color:st.color, fontWeight:'800', fontSize:12 }}>{st.icon} {st.label}</Text>
                      </View>
                    </View>
                    ...
                    {(() => {
                      ...
                      const parts:string[] = [];
                      if ((c.priceChanges||0)>0) parts.push(`price ${c.priceChanges}`);
                      if ((c.qtyDiffs||0)>0) parts.push(`qty ${c.qtyDiffs}`);
                      if ((c.unknown||0)>0) parts.push(`unknown ${c.unknown}`);
                      if ((c.missingOnInvoice||0)>0) parts.push(`missing ${c.missingOnInvoice}`);
                      return parts.length ? (
