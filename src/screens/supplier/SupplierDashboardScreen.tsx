...
function SupplierDashboardScreen({ supplierId }: { supplierId: string }) {
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: C.background }} contentContainerStyle={{ padding: 16, gap: 16 }}>
      {/* Header */}
      <View style={{ backgroundColor: C.primary, borderRadius: 16, padding: 20 }}>
      ...
      <View style={{ flexDirection: 'row', gap: 10 }}>
        {[
          { label: 'Pending orders', value: pending.length, colour: C.warning },
          { label: 'Total orders', value: orders.length, colour: C.accent },
          { label: 'Venues', value: account?.connectedVenues?.length || 0, colour: C.success },
        ].map((stat, i) => (
      ...
      {[
        ...
        <TouchableOpacity key={i} onPress={() => nav.navigate(item.route, { supplierId })}
          style={{ backgroundColor: C.surface, borderRadius: 14, padding: 16, flexDirection: 'row', alignItems: 'center', gap: 14, borderWidth: 1, borderColor: C.border }}>
          <Text style={{ fontSize: 24 }}>{item.icon}</Text>
          <Text style={{ fontWeight: '800', color: C.text, fontSize: 16 }}>{item.label}</Text>
          <Text style={{ marginLeft: 'auto', color: C.textSecondary, fontSize: 18 }}>›</Text>
        </TouchableOpacity>
      ))}
