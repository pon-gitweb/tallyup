    ...
    if (!confirm) {
      ...
      if (parsed?.hasPriceChanges) {
        navigation.navigate('InvoiceSummary', {
          ...
          invoiceNumber: parsed.invoiceNumber || null,
          productCount: parsed.lines?.length || 0,
          priceChanges: (parsed.proposals || [])
            .filter((p: any) => p.type === 'priceChange')
            .map((p: any) => ({
              name: p.productName,
              ...
              qty: p.qty,
              caseSize: p.caseSize,
              possibleCaseMismatch: p.possibleCaseMismatch,
              caseMismatchGuess: p.caseMismatchGuess,
              correctedUnitPrice: p.correctedUnitPrice,
              correctedChangePercent: p.correctedChangePercent,
