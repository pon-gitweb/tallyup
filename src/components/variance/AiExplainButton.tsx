...
import { View, Text, TouchableOpacity, Modal, Pressable, StyleSheet, ActivityIndicator, FlatList, Alert } from 'react-native';
...
export default function AiExplainButton({ departmentId=null, items=null, sinceDays=14, label }:Props){
  ...
  const [result, setResult] = useState<{explanation:string, bullets:string[]}>({ explanation:'', bullets:[] });
  ...
  const onPress = useCallback(async ()=>{
    ...
    // Hard-load gate: only show if entitled; otherwise show paywall
    const ok = await isEntitled(venueId, uid);
    if (!ok) { setPaywallOpen(true); return; }
  ...
  return (
    <View>
      <TouchableOpacity style={[S.btn, loading && S.btnDisabled]} onPress={onPress} disabled={loading}>
        {loading ? <ActivityIndicator/> : <Text style={S.btnText}>{label || '🤖 Explain variance'}</Text>}
      </TouchableOpacity>

      {/* Paywall (opens if not entitled) */}
      <PaymentSheet visible={paywallOpen} onClose={()=>setPaywallOpen(false)} />
