// Seed the 13 canonical NZ hospitality suppliers into global_suppliers.
// Safe to re-run — skips docs that already exist.
//
// Usage:
//   FIREBASE_PROJECT=tallyup-f1463 node scripts/seedGlobalSuppliers.cjs
//
...
(function init() {
  try {
    if (process.env.GOOGLE_APPLICATION_CREDENTIALS) {
      ...
      admin.initializeApp({ projectId: process.env.FIREBASE_PROJECT || 'tallyup-f1463' });
    }
  } catch (e) {
    // already initialised
  }
...
async function main() {
  const project = process.env.FIREBASE_PROJECT || 'tallyup-f1463';
  console.log(`Seeding global_suppliers → project: ${project}\n`);
