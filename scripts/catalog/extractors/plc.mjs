import { money, detectUnitAndSize, parseABV, cleanName } from '../utils.mjs';
export default function extractPLC({ text, supplier='Premium Liquor' }) {
  const rows = [];
  ...
  for (let i=0; i<lines.length; i++) {
    ...
    if (!size) continue;
    let name = cleanName(line.replace(/\s{2,}/g,' '));
